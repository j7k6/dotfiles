...
_git_status() {
  if [[ -d .git || $(git rev-parse --git-dir > /dev/null 2>&1) ]]; then
    git rev-parse --abbrev-ref HEAD > /dev/null 2>&1 && echo "%F{white}[git:$(git diff --no-ext-diff --quiet --exit-code && echo '%F{white} ' || echo '%F{red}')$(git rev-parse --abbrev-ref HEAD 2>&1)%f] %f"
  fi
}
